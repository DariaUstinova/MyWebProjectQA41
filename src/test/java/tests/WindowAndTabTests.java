...
public class WindowAndTabTests {
    ...
    public static  void sliderTest() throws InterruptedException { // Указание throws InterruptedException означает, что метод может выбрасывать исключение InterruptedException.
        ...
        WebDriver driver = new ChromeDriver(); // Создание экземпляра веб-драйвера Firefox. Этот драйвер будет использоваться для управления браузером.
...
//        Thread.sleep(3000);
    ...
    public static void switchTab(){
        ...
        scrollToElement(driver, newButton);
...
//        WebElement testElement = driver.findElement(By.xpath("//div[@id='finish']"));
//        testElement.click();
    ...
    public static String findRowByValue(String valueToFind){
        WebDriver driver = new ChromeDriver();
        try{ // try - if table doesn't exist
            driver.manage().window().maximize();
            ...
            for(WebElement row : rows){
                ...
                for(WebElement cell : cells){
                    if(cell.getText().equalsIgnoreCase(valueToFind)){
...
public static String findRowByValueLambda(String valueToFind){
    ...
    try{ // try - if table doesn't exist
        ...
        Optional<WebElement> optionalRow = rows.stream().filter(row ->row.findElements(By.tagName("td")).stream()
